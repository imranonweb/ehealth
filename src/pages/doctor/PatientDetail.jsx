...
import { User, Activity, Pill, Plus, ChevronLeft, Calendar, FileText, AlertCircle } from 'lucide-react';
...
export function DoctorPatientDetail() {
  ...
  useEffect(() => {
    async function loadPatientData() {
      try {
        ...
        // Fetch patient medical timeline
    ...
    return (
      <div className="dashboard-container">
        <div className="card" style={{ padding: 'var(--sp-8)', textAlign: 'center' }}>
          ...
          <p className="text-muted">The requested patient record could not be loaded.</p>
          <Link to="/doctor/patients" className="btn btn-outline btn-md" style={{ marginTop: 12 }}>
  ...
  return (
    <div className="dashboard-container">
      {/* Back button */}
      <Link to="/doctor/patients" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-3)', textDecoration: 'none', marginBottom: 16 }}>
      ...
      <div className="card" style={{ padding: 'var(--sp-6)', marginBottom: 'var(--sp-6)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
          ...
          <div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-3)', textTransform: 'uppercase', fontWeight: 600 }}>Contact Phone</div>
            <div style={{ fontSize: '0.9375rem', fontWeight: 600, color: 'var(--text-1)', marginTop: 2 }}>{patient.phone || '—'}</div>
          </div>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--sp-6)', borderBottom: '1px solid var(--border)', paddingBottom: 'var(--sp-4)' }}>
          <div>
            ...
            <p style={{ fontSize: '0.8125rem', color: 'var(--text-3)', marginTop: 2 }}>
              Historical diagnoses, hospitalizations, medications, and diagnostic investigations.
            </p>
          </div>
        </div>
